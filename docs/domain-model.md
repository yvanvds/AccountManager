...
- **INV-21 [D]:** Every WISA **person** appears in **exactly one** `LinkedAccount` — per person, not per row (#318, and see INV-11). A person the aggregated pull returns twice merges into one record whose `wisaSchoolIds` holds *both* schools, so `WisaPresence` reads `ours` when either of them is ours; the record's `wisa` — the row the class placement is derived from — is the row of a managed school, never whichever school the pull happened to read first (INV-25). Linking per row instead minted a second record for the same person, and since both rows key on `wisa:<wisaId>` the resolver handed the two records the **same** `LinkedAccountId`, putting two records' actions on one card.
...
- **INV-23 [D]:** When two Smartschool accounts collide on `mail`, **both** end up in some `LinkedAccount`, never silently dropped. A `ResolveDuplicateMail` warning action is raised. Keeping both means keeping two *records* for one person, so the pair also has to satisfy INV-24: the natural key falls through to the next unclaimed one, giving the extra co-account an id of its own (#323). It also means the Azure user whose `upn` matches that one mail has two records to attach to, and mail is the *weak* key: among the candidates still without an Azure user, the one whose WISA id equals the Azure `employeeId` wins, and only with none does the first in snapshot order take it (#354). The tie-break stays *within* the mail candidates — it never lets an `employeeId` reach a record the mail did not already point at — so a stale or mistyped one cannot steal an account the mail resolves correctly.
- **INV-24 [D]:** No two records in a `LinkedSnapshot` — `LinkedAccount` or `LinkedStaff` — share a `LinkedAccountId`. Enforced in `LinkedSnapshot.fromRecords` rather than in `link()`, so it holds for every snapshot built that way: a repeated id raises a `DuplicateLinkedId` warning naming the id and what each colliding record holds, and the id is counted once (#319). Every *known* cause is fixed at its source — INV-21 for the dual-enrolment row (#318), and the natural key's claim-and-fall-through for the duplicate-mail pair (#323) — so this is defence in depth. It earns its place because everything below the linker is keyed by that id and each layer degrades differently and silently: the materializer hands both records the **union** of their candidate actions, the Acties list is a last-wins map on `targetId`, and the shared store keeps one document per id, so one record is what every other operator inherits. The colliding records are **kept**, never deduped away — a silent drop is exactly what INV-23 exists to prevent, which is also why the duplicate-mail pair is fixed by re-keying the second record rather than by merging the two.

- **INV-25 [D]:** A person may appear in several group schools. **Our school's WISA row is authoritative for every value we write into our own systems** — the Smartschool class, the Office 365 class group, the account fields. A sibling school's row answers exactly one question: *is this person still somewhere in the group?* That answer gates deletion (#134) — Smartschool goes when they leave our school, Azure stays while they are anywhere in the group, and both go when they are not — and nothing else.

  The authoritative row is chosen **once**, by the linker (`_prefersWisaRow`, INV-21), and carried on `LinkedAccount.wisa`. Every layer below reads *that* row: looking a student up again by `wisaId` in a pooled index is how a sibling school's `3HWa` became a proposed Smartschool move for a student sitting in our `3MWW1` (#332), and linking per row instead of per person is how the sibling row got a card of its own to propose from (#318). Two bugs, one shape — a layer quietly using the other row — which is why the rule is written here rather than implied by the code that keeps it. `ClassPlacement.isOurClass` is the standing guard: a placement naming a class our own WISA school does not have is refused outright, whatever named it (#333).

  The sibling rows are not discarded. `LinkedAccount.wisaClassGroups` maps every school id the person was found in to the class *that* school holds them in, which is what `WisaPresence` is classified from and, since #334, what lets a view **state** the second enrolment — "Ook ingeschreven in Instituut Sancta Maria-B (ISMAB), klas 3HWa" on the account card, with the school named from the WISA school list (#204/#208). Stating it is the only other thing a sibling row may do: a dual enrolment is a state to be waited out, not an anomaly to be corrected, and a card that cannot say so reads as a bug. Writing a value read from there is the defect INV-25 exists to name.

- **INV-26 [I]:** **`employeeId` is not unique in the tenant.** It carries the WISA id and the linker treats it as the strong bridge to a person, but the live tenant holds pairs of Azure accounts answering to one id — audited Aug 2026 (#360): nine enrolled students of one school with two accounts each, the two UPNs differing only in how the given name was normalised, created months apart by two runs of this app. They are the fingerprint of a UPN-normalisation change, so more can appear. Any `Map<employeeId, account>` therefore keeps one row and loses the rest.

  So a person's *extra* Azure accounts are kept on `LinkedAccount.azureDuplicates` / `LinkedStaff.azureDuplicates` (the adopted one stays on `azure`; `azureCandidates` is the whole set, adopted first), and every colliding id raises a `DuplicateAzureEmployeeId` warning naming each account. `AzureSnapshot.duplicateEmployeeIds` is where the pull answers the question before anything has keyed by it — `UserManager.loadByEmployeeIds` already returns every match, de-duplicating by object id, so the collision is observable there and only vanishes further down. `UserManager.findByEmployeeId` still answers with *a* match rather than *the* match; that is sound only because its one caller is the create guard, which asks whether any account exists at all.

  Retaining the extra account is not cosmetic: an ambiguous identity is a third state beside "linked" and "missing", and both fates it used to have were wrong in the same way. Dropped, the pair was invisible. Kept as an Azure-only orphan — which it becomes whenever it carries our `companyName` — it read as a departed student and drew `RemoveStudentFromAzure`, on whichever of the two the join did not pick. The twins are indistinguishable to the join and one of them may hold the student's mail and OneDrive, so **resolution is the operator's**: the app reports the pair with the facts to choose by (UPN, object id, `accountEnabled`, the `companyName`/`jobTitle` pair the licence group's rule turns on, and — since #363 — when each account was created and when it was last signed into) and never merges or deletes.
...
| The co-account pair keys on one id (same `accountId`, or the shared mail) | Two records on one `LinkedAccountId`; consumers merge them | **Fix**: the natural key falls through to the first unclaimed one, so the extra co-account gets an id of its own | INV-23 + INV-24 (#323) |
| The Azure user's `upn` matches a mail two records share | First record with a free Azure slot takes it — snapshot order decides, and the `employeeId` bridge never runs | **Fix**: the WISA-anchored candidate (`wisaId == employeeId`) wins; snapshot order only breaks a tie among the rest | INV-23 (#354) |
...
| Two Azure accounts share an `employeeId` | One is picked by the join; the other is dropped, or kept as an orphan and proposed for deletion | **Fix**: the extras go on `azureDuplicates`, never onto an orphan record, and raise `DuplicateAzureEmployeeId`; the operator resolves it | INV-26 (#360) |
