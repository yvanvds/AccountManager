...
import 'package:smartschool_api/smartschool_api.dart'
    show SmartschoolSnapshot, smartschoolHttpFailure;
import 'package:wisa_api/wisa_api.dart'
    show DontImportUserFromWisa, WisaSchool, parseSchoolRow;
...
/// `Te laat` lives in `late_arrivals_test.dart`, `app updates` in
/// `app_update_test.dart` and `Instellingen` in `settings_test.dart`, and
...
///
///   * `app shell and log panel`   — launch, navigation rail, log panel
///   * `sign-in`                   — the AAD broker chain
///   * `Synchronisatie and the shared state`
///   * `Klasgroepen`               — the class inventory and its group writes
///   * `Acties`                    — the flat account list itself
///   * `Acties: leerlingen`        — student-side actions
///   * `Acties: personeel`         — staff-side actions
///   * `Azure and Office 365`      — delta pulls and Office 365 identity
///   * `duplicate accounts and id collisions`
///   * `Wachtwoorden`
///
...
void main() {
  ...
  group('app shell and log panel', () {
    ...
    testWidgets(
      ...
      expect(
          onScreen[1], '00:00:00  [smartschool]  Set-Password failed: rc=42');
  ...
  group('sign-in', () {
    ...
    testWidgets('an unavailable native broker falls through to interactive',
      ...
      await tester
          .pumpWidget(AccountManagerApp(session: session, graph: graph));
      await tester.pumpAndSettle();
  ...
  group('Synchronisatie and the shared state', () {
    testWidgets(
    ...
    testWidgets(
      ...
      expect(
          find.textContaining('leerling(en) opgehaald uit S1.'), findsWidgets);
      expect(
          find.textContaining('Geen rechtstreekse accounts in'), findsWidgets);
    ...
    testWidgets(
      ...
      expect(find.byKey(const ValueKey('reconcile-last-sync-wisa')),
          findsOneWidget);
    ...
    testWidgets(
      ...
      final DateTime yesterday =
          DateTime(now.year, now.month, now.day - 1, 8, 5);
    ...
    testWidgets(
      ...
      expect(find.byKey(const ValueKey('reconcile-category-students')),
          findsOneWidget);
      expect(find.byKey(const ValueKey('reconcile-category-staff')),
          findsOneWidget);
    ...
    testWidgets(
      ...
      expect(
          find.byKey(const ValueKey('reconcile-seed-refused')), findsNothing);
      expect(
        tester
            .widget<FilledButton>(find.byKey(const ValueKey('reconcile-sync')))
            .onPressed,
      ...
      await openKlasgroepen(tester);
      expect(
          find.byKey(const ValueKey('class-groups-read-only')), findsNothing);
      ...
      expect(
          find.byKey(const ValueKey('actions-shared-state')), findsOneWidget);
      expect(find.byIcon(Icons.lock_outline), findsNothing);
  ...
  group('Klasgroepen', () {
    ...
    testWidgets(
      ...
      expect(find.byKey(const ValueKey('entry-group-1V')), findsNothing,
          reason:
              'creating a virtual school\'s class is never worth proposing');
    ...
    testWidgets(
        ...
        'empty-class notice end-to-end (#222/#329)',
        (WidgetTester tester) async {
      ...
      expect(find.textContaining('Voeg deze klas toe aan Smartschool'),
          findsNothing,
    ...
    testWidgets(
      ...
      final saved = harness.soap.soapActions
          .where((a) => a.endsWith('#saveClass'))
          .length;
      expect(saved, 2);
      ...
      final summaries = harness.controller.applyResults!
    ...
    testWidgets(
      ...
      expect(find.descendant(of: rowOneA, matching: find.text('1')),
          findsOneWidget);
    ...
    testWidgets(
      ...
      expect(
        find.textContaining(
            'Werk het ledenbestand van GBS-2F bij (2 toevoegen'),
        findsOneWidget,
    ...
    testWidgets(
      ...
      expect(
        find.descendant(
          of: find.byKey(entry),
          matching:
              find.text('Maak de Office 365-groep GBS-5WW1 voor klas 5WW1'),
        ),
        findsOneWidget,
      );
      expect(
    ...
    testWidgets(
      ...
      final smartschool =
          find.byKey(const ValueKey('entry-choice-group-5WW1-1'));
      ...
      expect(
        find.descendant(
          of: office365,
          matching:
              find.text('description: ∅ → 5e jaar Wetenschappen-Wiskunde'),
        ),
        findsNothing,
      ...
      expect(
        find.descendant(
            of: smartschool,
            matching:
                find.text('Negeer deze klas bij het importeren uit WISA')),
        findsOneWidget,
      );
      expect(
        find.descendant(
    ...
    testWidgets(
      ...
      final office365 =
          find.byKey(const ValueKey('entry-outcomes-group-5WW1-0'));
      final smartschool =
    ...
    testWidgets(
      ...
      expect(find.descendant(of: row, matching: find.text(summary)),
          findsOneWidget);
      ...
      final Finder block =
          find.byKey(const ValueKey('entry-choice-group-1A-0'));
      ...
      expect(
          find.descendant(
              of: block, matching: find.text('leden verwijderen: 1')),
          findsOneWidget);
      expect(find.textContaining('∅ →'), findsNothing,
    ...
    testWidgets(
      ...
      final BuildContext context =
          tester.element(find.byType(ClassGroupsScreen));
    ...
    testWidgets(
      ...
      expect(
          find.byKey(const ValueKey('actions-class-attention')), findsNothing);
    ...
    testWidgets(
      ...
      expect(
        find.textContaining(
            'Werk het ledenbestand van GBS-1B bij (1 toevoegen'),
        findsWidgets,
      );
    ...
    testWidgets(
        'an Office 365 class group is brought back to exactly its WISA class, '
        'end-to-end (#385, #389)', (WidgetTester tester) async {
      // `GBS-1A` holds four accounts and WISA puts exactly one of them in 1A:
      // Jane. The other three are the three ways a stray gets in — Tom, gone from
      // WISA altogether and flagged for deletion (#385); Anna, the class titular;
      // and `az-sda`, a sibling group school's pupil who is in no snapshot at all
      // because the Azure read is scoped to our own prefix (#389).
      //
      // #385 could only reach Tom: its net was built from linked records, and
      // `az-sda` has none. #389 made the roster itself the rule, so all three go.
      // Anna goes with them by design — a teacher who is a *member* of a class
      // group is a mistake, and one who needs access is made an owner, which this
      // app never reads.
      //
      ...
      // "and `az-sda` was actually deleted" is a fact about a write rather than a
      // plan.
      useTallWindow(tester);
      ...
      // The class row proposes all three removals — and nothing to add, because
      // the one member the roster does name is already there.
      expect(
        find.textContaining(
            'Werk het ledenbestand van GBS-1A bij (0 toevoegen, 3 verwijderen)'),
        findsWidgets,
      );
      ...
      // left of them.
      final String tomId =
          accountId(harness, 'tom.sels@student.school.example');
      ...
      // Apply the class's roster sync and watch the wire: three membership
      // writes, and the one account WISA puts in 1A is not among them.
      await openKlasgroepen(tester);
      ...
      expect(harness.graph.batchedWrites, hasLength(3));
      for (final id in const ['az-tom', 'az-anna', 'az-sda']) {
        expect(
          harness.graph.batchedWrites
              .any((w) => w.startsWith('DELETE /groups/az-GBS-1A/members/$id')),
          isTrue,
          reason: '$id is not in the WISA class, so it is not in the group',
        );
      }
      expect(
        harness.graph.batchedWrites
            .any((w) => w.startsWith('DELETE /groups/az-GBS-1A/members/az1')),
        isFalse,
        reason:
            'the one student who genuinely belongs there is untouched by the '
            'write the rollover runs on every class at once',
      );
      expect(tester.takeException(), isNull);
    });
    ...
    testWidgets(
      ...
      expect(
        find.textContaining(
            'Werk het ledenbestand van GBS-1A bij (1 toevoegen'),
        findsWidgets,
      );
      expect(
          find.textContaining('mail-enabled beveiligingsgroep'), findsNothing);
      expect(tester.takeException(), isNull);
    });
    ...
    testWidgets(
      ...
      expect(
        ...
        isEmpty,
        reason:
            'Office 365 class membership is a property of the group, so the '
  ...
  group('Acties', () {
    ...
    // headers, which the class-inventory scenarios in the Klasgroepen group
    // drive directly, and — on Acties since #296 — a per-decision "Toepassen op
    // alle" that filters the flat list down to its cohort before offering a
    // confirmation. The run below is that one, end to end.
    ...
    testWidgets(
      ...
      expect(
        ...
        containsAll(
            <ActionOutcome>[ActionOutcome.applied, ActionOutcome.failed]),
      );
      expect(
    ...
    testWidgets(
      ...
      final Finder apply =
          find.byKey(ValueKey('entry-apply-${entry.targetId}'));
      await tester.ensureVisible(apply);
      await tester.tap(apply);
      await tester.pumpAndSettle();
      await tester.tap(find.byKey(const ValueKey('actions-apply-confirm')));
    ...
    testWidgets(
      ...
      for (final klas in const <String>['1A', '1B', '3C', 'OKAN']) {
        ...
        expect(find.descendant(of: row, matching: find.text(klas)),
            findsOneWidget);
      }
      ...
      final harness = ReconcileHarness(
        wisa: wisaSnap(
          ...
          classGroups: [
            ...
            wisaClassGroup('1C',
                adminCode: 'a2', schoolCode: '222', schoolId: 2),
    ...
    testWidgets(
      ...
      expect(find.descendant(of: movedRow, matching: find.text('3D')),
          findsOneWidget);
      expect(find.descendant(of: movedRow, matching: find.text('3C')),
          findsNothing);
    ...
    testWidgets(
      ...
      expect(find.byKey(ValueKey('account-done-$sam')), findsOneWidget);
      expect(
          find.descendant(of: samRow, matching: find.text('1')), findsNothing,
    ...
    testWidgets(
      ...
      expect(
          find.byKey(const ValueKey('actions-tab-personeel')), findsOneWidget);
    ...
    testWidgets(
      ...
      await tester.pumpAndSettle();
      expect(find.text('Geen accounts die aan de filter voldoen.'),
          findsOneWidget);
  ...
  group('Acties: leerlingen', () {
    ...
    testWidgets(
        ...
        'and is remembered across a restart (#394)',
        (WidgetTester tester) async {
      ...
      expect(first.soap.unregisteredOn, <String>[onWire],
          reason:
              'the wire value is the point of the issue — not today, and not '
    ...
    testWidgets(
      ...
      expect(kinds, isNot(contains('UnregisterStudentFromSmartschool')),
          reason:
              'the student is enrolled with us — never offer to unregister');
      ...
      expect(
          find.descendant(of: row, matching: find.text('4ECO')), findsNothing);
      expect(find.descendant(of: row, matching: find.text('Zonder klas')),
          findsNothing);
    ...
    for (final MapEntry<String, bool> order in <String, bool>{
      ...
      testWidgets(
        ...
        await tester.pumpAndSettle();
        await tester
            .tap(find.byKey(const ValueKey('actions-only-with-actions')));
        await tester.pumpAndSettle();
    ...
    testWidgets(
      ...
      for (final school in <String>['1', '2']) {
        ...
        expect(row, findsOneWidget);
        await tester.ensureVisible(row);
        expect(find.descendant(of: row, matching: find.text('1C')),
            findsOneWidget);
    ...
    testWidgets(
      ...
      expect(find.text('Wijzig de klas in Smartschool'), findsNothing);
      expect(
          find.text('Wijzig het stamboeknummer in Smartschool'), findsWidgets);
    ...
    testWidgets(
      ...
      expect(
          harness.soap.soapActions.any((a) => a.contains('saveUser')), isTrue);
      ...
      expect(
          messages, contains(contains('Maak een nieuw Smartschool account')));
    });
    ...
    testWidgets(
      ...
      expect(
        ...
        isNotNull,
        reason:
            'a create reported as failed spliced nothing, and the card then '
    ...
    testWidgets(
      ...
      expect(
        find.descendant(
            of: verdict,
            matching: find.text('Wijzig de functietitel in Azure')),
        findsOneWidget,
      );
      expect(
        harness.controller.pendingEntries.where((e) => e.family == 'student'),
    ...
    testWidgets(
      ...
      final List<MaterializedAccount> stranded =
          await linkedStore.readClassroom(
  ...
  group('Acties: personeel', () {
    ...
    testWidgets(
      ...
      expect(harness.soap.movedToClasses, isEmpty,
          reason:
              'a staff seat is a group membership, never an official class');
    ...
    testWidgets(
      ...
      expect(
          harness.app.azure.snapshot!.users.single.department, 'SSM,KAV,GBS');
    ...
    testWidgets(
      ...
      final Finder tile =
          find.byKey(const ValueKey('reconcile-category-staff'));
      await tester.ensureVisible(tile);
      expect(find.descendant(of: tile, matching: find.text('PERSONEEL')),
          findsOneWidget);
      expect(
          find.descendant(of: tile, matching: find.text('2')), findsOneWidget);
      ...
      expect(
        harness.log.entries.map((e) => e.message),
        contains(
            '1 personeelslid/-leden overgeslagen: niet in een school die we '
            'beheren.'),
      );
      ...
      expect(carla.hasLeftGroup, isFalse,
          reason: 'the group still employs her');
      expect(
      ...
      expect(
        harness.controller.linked!.staffActions
            .whereType<RemoveStaffFromAzure>(),
        isEmpty,
        reason:
            'deleting it would destroy the account of somebody WISA can see '
      ...
      expect(
        ...
        <String>['SSM'],
        reason:
            "our prefix struck out, the sibling school's entry kept verbatim",
      );
      ...
      expect(
        ...
        reason:
            'the one person of ours with work — never Bert, whose school we '
            'do not manage',
      );
    ...
    testWidgets(
      ...
      final String id =
          harness.controller.linked!.snapshot.staff.single.id.value;
      await selectAccount(tester, id);
    ...
    testWidgets(
      ...
      expect(find.text(line), findsOneWidget,
          reason:
              'the details pane only — an extra line per collapsed card on a '
  ...
  group('Azure and Office 365', () {
    testWidgets(
      ...
      await tester.pumpAndSettle();
      expect(
          azureWire.resumeTokens, <String>['DEADTOKEN', 'FRESH-DELTA-TOKEN']);
    ...
    testWidgets(
      ...
      expect(
          find.text('Wijzig het e-mailadres in Smartschool'), findsOneWidget);
    });
  ...
  group('duplicate accounts and id collisions', () {
    testWidgets(
    ...
    testWidgets(
      ...
      expect(
        harness.log.entries.map((e) => e.message).where(
            (m) => m.contains('Koppelingsfout') && m.contains('p-shared')),
        hasLength(1),
      );
    ...
    testWidgets(
      ...
      expect(
        find.descendant(
            of: tile, matching: find.textContaining('los dit op in')),
        findsOneWidget,
      );
    ...
    testWidgets(
      ...
      expect(allKinds, isNot(contains('RemoveStudentFromAzure')),
          reason:
              'a staff-stamped account must never reach the student delete');
    ...
    testWidgets(
      ...
      expect(
        find.byKey(
            const ValueKey('dup-warning-jane.doe@student.school.example')),
        findsOneWidget,
      );
  ...
  group('Wachtwoorden', () {
    testWidgets(
      ...
      await tester.pumpAndSettle();
      expect(find.byType(PasswordsScreen), findsOneWidget);
      expect(find.byKey(const ValueKey('passwords-tab-leerlingen')),
          findsOneWidget);
      expect(find.byKey(const ValueKey('passwords-tab-personeel')),
          findsOneWidget);
    ...
    testWidgets(
      ...
      await search('BRAV');
      expect(
          find.byKey(const ValueKey('passwords-staff-alice')), findsOneWidget);
      expect(find.byKey(const ValueKey('passwords-staff-bob')), findsNothing);
      expect(
          find.byKey(const ValueKey('passwords-staff-charlie')), findsNothing);
    ...
    testWidgets(
      ...
      await tester.pumpAndSettle();
      await tester
          .tap(find.byKey(const ValueKey('passwords-staff-reset-o365')));
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 300));
      await tester
          .tap(find.byKey(const ValueKey('passwords-staff-reset-confirm')));
    ...
    testWidgets(
        ...
        'the column instead of clearing it (#376)',
        (WidgetTester tester) async {
